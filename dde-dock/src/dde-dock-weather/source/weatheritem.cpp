...
void WeatherItem::paintEvent(QPaintEvent *e) {
    ...
    if (displayMode == Dock::Efficient) {
        ...
        painter.drawPixmap(rf.width()/4, 3, m_iconPixmap);
        int fontSize = std::min(width(), height()) * 0.24;
        QFont font = qApp->font();
        ...
        painter.setRenderHint(QPainter::Antialiasing);
        painter.translate(width() / 2, height() - fontSize * 1.5);
...
void WeatherItem::refreshIcon() {
    ...
    iconSize = std::min(width(), height()) * 0.6;
